<?php
	...
	$UserRegisteredOn= $_REQUEST["userRegisteredOn"];
	...
	if(mysqli_num_rows($result) > 0){
		...
		$sql="UPDATE auth SET userRegisteredOn = '".$UserRegisteredOn."' WHERE macAddressWlan0 = '".$MacAddressWlan0."'";
		$result = mysqli_query($con, $sql);
		if($result){die("success");}
		else{die("failed");}
	}else{
		$sql = "INSERT INTO auth(macAddressWlan0, userRegisteredOn) VALUE('$MacAddressWlan0', '$UserRegisteredOn')";
		$result = mysqli_query($con, $sql);
		if($result){die("success");}
		else{die("failed");}
	}
